...
def cities_game(update, context):   #функция игры в города
    ...
    try:
        game_list=(user_game_list(context.user_data))         #создаем пустой списк для игнры (необходимо заменить на функцию user_data )
        file = pd.read_csv('city.csv')          #создаем список со всеми городами
        all_cities = file['city'].tolist()
        rest_all_cities=set(all_cities)-set(game_list) #вычисляем города, которые остались для игры
        user_word = " ".join([word.capitalize() for word in user_word]) # приводим к заглавной первой буквеб введенный текст
        ...
        if user_word not in rest_all_cities:   # Проверяем первый раз ли вводится город
        ...
        elif len(game_list)>=1 and game_list[-1][-1]!=user_word.lower()[0]: # Проверяем корректность первой буквы
            ...
            rest_all_cities-=set(game_list) # исключаем слово
    ...
    except (TypeError, ValueError):           # !!! исключения ???
        update.message.reply_text("Вы указали название города некорректно.")
